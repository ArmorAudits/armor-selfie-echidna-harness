...
def list_repos_for(owner):
    ...
    for kind in ("orgs", "users"):
        page = 1
        while True:
            r = _get(f"https://api.github.com/{kind}/{owner}/repos",
...
def read_blob(owner, repo, path, ref):
    r = _get(f"https://api.github.com/repos/{owner}/{repo}/contents/{path}", params={"ref":ref})
...
def main():
    ...
    for who in [x.strip() for x in args.orgs.split(",") if x.strip()]:
        ...
        for r in repos:
            ...
            cand = [b for b in blobs if (PAT.search(b["path"]) or b.get("path","").endswith((".sol",".vy",".move",".cairo")))
            ...
            for b in cand:
                ...
                found = detect(txt)
                for name in found:
                    discovered.append({"Tool":name,"Repo":f"{owner}/{repo}","Path":b["path"]})
                    seen.add(name)

    disc_csv = os.path.join(args.outdir,"github_tools_detected.csv")
    with open(disc_csv,"w",newline="",encoding="utf-8") as f:
        w=csv.DictWriter(f,fieldnames=["Tool","Repo","Path"]); w.writeheader()
        for row in sorted(discovered,key=lambda x:(x["Tool"],x["Repo"],x["Path"])): w.writerow(row)
    ...
    emit(os.path.join(args.outdir,"diff_missing_in_inventory.csv"),
         missing, "Tool (found in GitHub, NOT in tools.csv)")
    emit(os.path.join(args.outdir,"diff_not_seen_in_github.csv"),
         not_seen,"Tool (in tools.csv, NOT detected in GitHub)")
