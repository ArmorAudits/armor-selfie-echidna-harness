...
def audit():
    ...
    for r in rows:
        ...
        allow_missing_repo = str((r.get("AllowMissingRepo") or "")).strip().lower() in (
            "1",
            "true",
            "yes",
            "y",
        ...
        force_not_archived = str((r.get("ForceNotArchived") or "")).strip().lower() in (
            "1",
            "true",
            "yes",
            "y",
    ...
    allowed_arch = [
        s
        for s in out
        if ("no longer supported" in " ".join(s["reasons"])) and not s["quarantine"]
    ]
    if allowed_arch:
        lines.append("\n## Allowed (archived)\n\n")
        lines.append(
    ...
    for s in out:
        rep.append(
            f"| {s['tool']} | {s['repo']} | {s['pinned']} | {s['latest']} | {'; '.join(s['reasons']) or '-'} | {'YES' if s['quarantine'] else 'no'} |\n"
        )
    REPORT.write_text("".join(rep), encoding="utf-8")

    STATUS.write_text(
        json.dumps({"generated_at": ts, "items": out}, indent=2), encoding="utf-8"
    )
