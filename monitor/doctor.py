...
def gh_latest_release(repo_url: str, token: str) -> Dict[str,Any]:
    ...
    # repo meta (archived?)
    r = s.get(f"https://api.github.com/repos/{owner}/{repo}", headers=headers, timeout=15)
    if r.ok:
        j = r.json()
        out["archived"] = bool(j.get("archived"))
    # releases
    r = s.get(f"https://api.github.com/repos/{owner}/{repo}/releases/latest", headers=headers, timeout=15)
    if r.status_code == 404:
        # try tags
        r2 = s.get(f"https://api.github.com/repos/{owner}/{repo}/tags?per_page=1", headers=headers, timeout=15)
            ...
            for r in j.get("results", []):
                for pkg in r.get("packages", []):
                    for v in pkg.get("vulnerabilities", []):
                        s = (v.get("severity") or v.get("database_specific",{}).get("severity") or "").upper()
                        if not s and v.get("severity"):
                            if isinstance(v["severity"], list) and v["severity"]:
                                s = (v["severity"][0].get("type") or v["severity"][0].get("score") or "").upper()
                        if "CRITICAL" in s: bump("CRITICAL")
                        elif "HIGH" in s:   bump("HIGH")
                        elif "MEDIUM" in s: bump("MEDIUM")
                        elif "LOW" in s:    bump("LOW")
        except Exception:
            pass
...
def decide_status(tool: Dict[str,Any], latest: Dict[str,Any], sev: str) -> Dict[str,Any]:
    status = "Adopted"
    status_on = now_iso()
    reason_code = ""
    reason = ""
    ...
    if latest.get("archived"):
        return {"Status":"Quarantined","StatusOn":status_on,"ReasonCode":"ARCHIVED","Reason":"Upstream repo archived"}
    ...
    if sev != "NONE" and SEV_ORDER.get(sev,0) >= SEV_ORDER.get(POLICY_MIN_SEV,3):
        return {"Status":"Quarantined","StatusOn":status_on,"ReasonCode":"VULN","Reason":f"Max severity {sev} ≥ policy {POLICY_MIN_SEV}"}

    return {"Status":status,"StatusOn":status_on,"ReasonCode":reason_code,"Reason":reason}
...
def main():
    ...
    for t in inv:
        ...
        row = {
            **t,
            "Version": latest or "",
            "LastUpdateCheck": now_iso(),
            **status
        }
        all_rows.append(row)

    # CSV header
    cols = ["Tool","Category","Chain","Vendor","License","RepoURL","MonitorMethod",
            "InstallMethod","BinaryPath","Version","FirstSeen","AdoptedOn",
            "LastUsed","LastUpdateCheck","Status","StatusOn","ReasonCode","Reason",
